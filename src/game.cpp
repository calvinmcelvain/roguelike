...
void Game::run() {
    std::cout << "Roguelike Game Started! Use arrow keys to move. Press Q to quit.\n";
...
void Game::handleInput() {
    if (_kbhit()) {
        ...
        case 224: // Extended key
        {
            ...
            if (arrowKey == 72) newPos.y--;      // Up
            else if (arrowKey == 80) newPos.y++; // Down
            else if (arrowKey == 75) newPos.x--; // Left
            else if (arrowKey == 77) newPos.x++; // Right
            
            if (level.isWalkable(newPos)) {
                if (arrowKey == 72) player.moveUp();      // Up
                else if (arrowKey == 80) player.moveDown(); // Down
                else if (arrowKey == 75) player.moveLeft(); // Left
                else if (arrowKey == 77) player.moveRight(); // Right
            }
            break;
        }
        }
